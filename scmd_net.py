...
Complete model orchestrating LSC and MBD stages.
"""
...
class SCMDNet(nn.Module):
    """
    SCMD-Net architecture:
    
    Stage 1 - LSC (Latent Semantic Clustering):
        1. Multi-modal encoding (DeBERTa, Ma-Net, Wav2Vec)
        2. Context enhancement with K-layer RoPE Transformer
        3. Aura Vector extraction (global mean pooling + FFN)
        4. Dual-head optimization (Task Head + Aura Head)
    
    Stage 2 - MBD (Multi-modal Bridging Decoupling):
        1. Semantic Key Calibration (Aura -> Key space projection)
        2. Adaptive Residual Injection (sigmoid-gated fusion)
        3. Concatenation + classification head
    """
    ...
    def __init__(self, config: dict):
        ...
        # ========== Context Transformers (per modality) ==========
        ...
        # ========== LSC Stages (per modality) ==========
        self.text_lsc = LSCStage(
            input_dim=transformer_dim, aura_dim=aura_dim,
            num_classes=num_classes, dropout=dropout
        )
        self.visual_lsc = LSCStage(
            input_dim=transformer_dim, aura_dim=aura_dim,
            num_classes=num_classes, dropout=dropout
        )
        self.audio_lsc = LSCStage(
            input_dim=transformer_dim, aura_dim=aura_dim,
            num_classes=num_classes, dropout=dropout
        )
    ...
    def encode_visual(self, visual_frames: torch.Tensor,
        ...
        if frame_mask is None:
            frame_mask = torch.ones(visual_frames.shape[:2], 
                                    device=visual_frames.device)
    ...
    def encode_audio(self, audio_values: torch.Tensor,
        ...
        # Create mask for audio features
        if audio_mask is not None:
            # Resize mask to match feature sequence length
            feature_len = seq_features.shape[1]
            original_len = audio_mask.shape[1]
            if feature_len != original_len:
                # Use interpolation for proper resizing
            ...
            audio_mask = torch.ones(seq_features.shape[:2],
    ...
    def lsc_forward(self, seq_features: torch.Tensor, mask: torch.Tensor,
                    labels: torch.Tensor, lsc_stage: LSCStage,
                    modality_idx: int) -> dict:
        """
        Run LSC stage for a single modality.
        """
        aura, task_logits, aura_logits = lsc_stage(seq_features, mask)
        task_loss, aura_loss = lsc_stage.compute_loss(
            aura, task_logits, aura_logits, labels, mask
        )
        weighted_loss = lsc_stage.get_weighted_loss(
            task_loss, aura_loss, modality_idx,
            self.task_weight, self.aura_weight
        )
        return {
            "aura": aura,
            "task_logits": task_logits,
            "aura_logits": aura_logits,
            "task_loss": task_loss,
            "aura_loss": aura_loss,
    ...
    def forward(
        ...
        Returns:
            Dictionary containing:
                - fused_logits: (B, num_classes) final prediction
                - aura_vectors: dict of aura vectors per modality
                - lsc_results: dict of LSC outputs per modality
                - total_loss: scalar total loss (if labels provided)
                - task_logits: dict of per-modality task logits
        """
        # ========== Stage 1: Multi-modal Encoding + Context Enhancement ==========
        text_features, text_mask = self.encode_text(
            text_input_ids, text_attention_mask
        )
        visual_features, visual_mask = self.encode_visual(
            visual_frames, visual_frame_mask
        )
        audio_features, audio_mask = self.encode_audio(
        ...
        lsc_results = {}
        task_logits = {}
        total_lsc_loss = torch.tensor(0.0, device=text_features.device)
        ...
        for idx, modality_name in enumerate(modality_order):
            ...
            if labels is not None:
                total_lsc_loss = total_lsc_loss + result["weighted_loss"]

        # ========== Stage 2: MBD (Multi-modal Bridging Decoupling) ==========
        fused_logits, fused_features, fusion_loss = self.mbd_fusion(
            seq_features, aura_vectors, seq_masks
        )
