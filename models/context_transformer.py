...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, seq_len: int, offset: int = 0):
        t = torch.arange(offset, offset + seq_len, device=self.inv_freq.device).type_as(self.inv_freq)
...
class ContextTransformerBlock(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int = 8, ff_ratio: float = 4.0, dropout: float = 0.1):
        super().__init__()
...
class ContextTransformer(nn.Module):
    """K-layer Transformer with Rotary Position Encoding for modality-specific context enhancement."""

    def __init__(self, dim: int, num_layers: int = 4, num_heads: int = 8, dropout: float = 0.1):
        super().__init__()
